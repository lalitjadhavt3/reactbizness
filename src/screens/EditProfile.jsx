...
const EditProfile = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (response?.data) {
        setData(response?.data?.data[0]); // navigate('/'); // Optionally, redirect or update UI to indicate successful login
      } else {
  ...
  const handleUpdate = () => {
    ...
    if (
      ...
      alert('Change Something nigga');
    } else {
      Formdata = Object.keys(Formdata)
        .filter((objKey) => Formdata[objKey] != null)
        .reduce((newObj, key) => {
